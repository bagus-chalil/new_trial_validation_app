...
function AppIcon({ item, onClose }: { item: AppItem; onClose: () => void }) {
    ...
    const inner = (
        <div className="flex flex-col items-center gap-2">
            ...
            <span className="w-[70px] text-center text-[11.5px] leading-tight font-semibold">{label}</span>
        </div>
    );
    if (href)
        return (
            <Link href={href} onClick={onClose} className="transition-transform active:scale-95">
                {inner}
            </Link>
        );
    return (
        ...
        <Sheet open={open} onOpenChange={(v) => !v && handleClose()}>
            <SheetContent side="bottom" className="rounded-t-[28px] border-t-0 p-0 pb-[env(safe-area-inset-bottom,0px)]">
                ...
                <div className="border-border flex items-center gap-3 border-b px-5 py-3.5">
                    <div className="bg-primary/10 text-primary flex size-[38px] shrink-0 items-center justify-center rounded-full text-[13px] font-bold">
                        {initials}
                    </div>
                ...
                <div className="flex flex-col gap-5 px-5 py-5">
                    {sections.map((section) => (
                        <div key={section.title} className="flex flex-col gap-3">
                            <span className="text-muted-foreground text-[11px] font-semibold tracking-widest uppercase">{section.title}</span>
                            <div className="grid grid-cols-4 gap-y-4">
                                {section.items.map((item) => (
                                    <AppIcon key={item.label} item={item} onClose={handleClose} />
                                ))}
                            </div>
                        </div>
                    ))}
